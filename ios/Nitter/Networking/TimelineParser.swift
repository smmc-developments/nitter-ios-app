...
enum TimelineParser {
    ...
    private static func videoPoster(in body: Element, baseURL: URL) throws -> URL? {
        for attachments in ownAttachments(in: body) {
            for attachment in try attachments.select(".attachment") {
                let video = try attachment.select("video").first()
                let download = try attachment.select("a.video-download").first()
                guard video != nil || download != nil else { continue }
                let poster = try video?.attr("poster")
                    ?? attachment.select("img").first()?.attr("src")
                    ?? ""
                if !poster.isEmpty {
        ...
        for attachments in ownAttachments(in: body) {
            for attachment in try attachments.select(".attachment") {
                ...
                let source = try video?.select("source[type=video/mp4]").first()?.attr("src")
                    ?? video?.select("source").first()?.attr("src")
                    ?? video?.attr("src")
                    ?? download?.attr("href")
                    ?? ""
                if !source.isEmpty {
                    return playbackURL(source, baseURL: baseURL)
                }
            }
        }
        return nil
    ...
    private static func playbackURL(_ path: String, baseURL: URL) -> URL? {
        guard let instanceURL = absoluteURL(path, baseURL: baseURL) else { return nil }
        let prefix = "/video/"
        guard let encodedPath = URLComponents(
            url: instanceURL,
            resolvingAgainstBaseURL: false
        )?.percentEncodedPath else { return instanceURL }
        guard encodedPath.hasPrefix(prefix) else { return instanceURL }
        let remainder = encodedPath.dropFirst(prefix.count)
        guard let separator = remainder.firstIndex(of: "/"),
              let decoded = String(remainder[remainder.index(after: separator)...]).removingPercentEncoding,
              let directURL = URL(string: decoded),
              directURL.scheme == "https",
              directURL.host == "video.twimg.com",
              directURL.port == nil,
              directURL.user == nil,
              directURL.password == nil else {
            return instanceURL
        }
        return directURL
    }
