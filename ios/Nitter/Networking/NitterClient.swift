...
/// with its JavaScript anti-bot challenge (HTTP 403 or 503).
actor NitterClient {
    ...
    private let baseURL = "https://nitter.click"
    ...
    /// Spaces request starts apart so rapid per-account fetches do not trip
    /// the instance's rate limiter. Actor isolation makes this race-free.
    private func waitForRequestSlot() async throws {
        let now = Date()
        if nextRequestAt > now {
            try await Task.sleep(for: .seconds(nextRequestAt.timeIntervalSince(now)))
        }
        nextRequestAt = Date().addingTimeInterval(Self.baseRequestDelay)
    }
