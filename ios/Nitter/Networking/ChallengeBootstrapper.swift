...
final class ChallengeBootstrapper {
    ...
    private var solveTask: Task<Bool, Never>?
    ...
    /// Returns `true` when a session is ready. Concurrent callers await the
    /// same solve instead of each starting their own hidden web view.
    @discardableResult
    func ensureSession(path: String) async -> Bool {
        if let solveTask {
            return await solveTask.value
        }
        let task = Task { @MainActor [weak self] in
            guard let self else { return false }
            return await self.solve(path: path)
        }
        solveTask = task
    ...
    private func solve(path: String) async -> Bool {
        ...
        guard let url = URL(string: "https://nitter.click" + path) else { return false }
