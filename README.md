...
Twitter/X content is accessed through a public Nitter instance, defaulting to nitter.click. The companion server launches a real system Chrome browser via Chrome DevTools Protocol and uses Playwright for challenge-aware timeline and media requests.
...
- **Tweet Detail** — Full tweet view with replies, parent context, and in-app navigation
...
| `GET` | `/api/tweet/:username/:id` | Bearer | Tweet detail with replies |
...
Nitter instances apply different anti-bot controls. The server launches a real system Chrome with `--remote-debugging-port`, connects via Playwright's `connectOverCDP`, bootstraps the configured instance in the browser, and then uses the browser's HTTP context for timeline requests.

On macOS, Chrome runs with the native display. On Linux/Docker, `xvfb` provides a virtual display.

### Feed Scheduling

Accounts are prioritized by activity: new > active (24h) > warm (7d) > dormant (7d+). Repeated upstream failures back off from one hour to one day. Each account tracks its own cursor position for pagination, and tweets not observed for 30 days are pruned.
...
The server proxies Nitter media through the browser context with signed HMAC URLs (stable for ~24 hours). The iOS app uses `CachedAsyncImage` with request coalescing for efficient loading.
